...
{
    public class BybitInstrumentManager : BaseInstrumentManager<BybitInstrumentViewModel, BybitInstrument>
    {
        private readonly Bybit _connection;
        ...
        public BybitInstrumentManager(Bybit connection) : base(connection)
        {
            _connection = connection;
            InstrumentColumnCustomization = SystemOptions.Instance.ViewOptions["BybitInstruments"];
        }


        public Instrument BybitInstrumentToInstrument(BybitInstrumentViewModel providerInstrumentVm)
        {
            return ProviderInstrumentToInstrument(providerInstrumentVm);
        }

        protected override Instrument ProviderInstrumentToInstrument(BybitInstrumentViewModel providerInstrumentVm)
        {
        ...
        {
            var instrument = item as BybitInstrumentViewModel;
            if (instrument == null)
                return false;
        ...
        public override List<Instrument> ConvertToInstruments(List<object> insCollection, string type, List<string> symbolList = null)
        {
            ...
            var binanceInstruments = insCollection.ConvertAll(x => (BybitInstrument)x);
            ...
            {
                instruments.Add(BybitInstrumentToInstrument(new BybitInstrumentViewModel(instrument)));
            }
            return instruments;
        }
        ...
        public override async Task FetchInstruments()
        {
            try
            {
                ...
                if (_connection is IDataProvider dataProvider)
                {
                    ...
                    var instruments = insCollection.ConvertAll(x => (BybitInstrument)x);
                    ...
                    {
                        Instruments.Add(new BybitInstrumentViewModel(instrument) { ConnectionName = _connection.Name });
                    }
