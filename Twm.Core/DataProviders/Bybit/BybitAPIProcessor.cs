...
namespace Twm.Core.DataProviders.Bybit
{
    ...
    /// </summary>
    public class BybitAPIProcessor : IAPIProcessor
    {
        ...
        public BybitAPIProcessor(string apiKey, string secretKey, IAPICacheManager apiCache)
        {
            _apiKey = apiKey;
        ...
        private async Task<T> ProcessJsonResponse<T>(HttpResponseMessage message, string requestMessage, string fullCacheKey) where T : class
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new BybitException(deserializeErrorMessage, new BybitError()
                {
        ...
        {
            BybitError errorObject = null;
            BybitException exception;
            ...
            if (responseType == "json")
            {
                try
                {
                    ...
                    errorObject = JsonConvert.DeserializeObject<BybitError>(errorJson);
                }
                catch
                {
                    errorObject = new BybitError();
                    errorObject.RequestMessage = requestMessage;
                    errorObject.Message = "Can`t read response";
                    exception = CreateBybitException(message.StatusCode, errorObject);
                    throw exception;
                }

                if (errorObject == null)
                    throw new BybitException("Unexpected Error whilst handling the response", null);
                errorObject.RequestMessage = requestMessage;
                exception = CreateBybitException(message.StatusCode, errorObject);
                throw exception;
            }
        ...
        private BybitException CreateBybitException(HttpStatusCode statusCode, BybitError errorObject)
        {
            if (statusCode == HttpStatusCode.GatewayTimeout)
            {
                return new BybitTimeoutException(errorObject);
            }
            if (statusCode == HttpStatusCode.Unauthorized)
            {
                return new BybitUnauthorizedException(errorObject);
            }
            var parsedStatusCode = (int)statusCode;
            if (parsedStatusCode >= 400 && parsedStatusCode <= 500)
            {
                return new BybitBadRequestException(errorObject);
            }
            return parsedStatusCode >= 500 ? 
                new BybitServerException(errorObject) : 
                new BybitException("Bybit API Error", errorObject);
        }
